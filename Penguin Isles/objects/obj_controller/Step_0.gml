...
// Log camera settings every ~60 steps (approx 1 second) to avoid spam
if ((current_time div 1000) mod 60 == 0) { // Rough check every 60 frames
    if (variable_global_exists("camera") && camera_exists(global.camera)) {
        var _border_x = camera_get_view_border_x(global.camera);
        var _border_y = camera_get_view_border_y(global.camera);
        var _speed_x = camera_get_view_speed_x(global.camera);
        var _speed_y = camera_get_view_speed_y(global.camera);

        if (_border_x != 0 || _border_y != 0 || _speed_x != -1 || _speed_y != -1) {
            show_debug_message("CONTROLLER STEP CHECK (WARNING): Camera border=(" + string(_border_x) + "," + string(_border_y) + "), Speed=(" + string(_speed_x) + "," + string(_speed_y) + ")");
        } else {
            // Optional: Log confirmation that settings are correct
            // show_debug_message("CONTROLLER STEP CHECK (OK): Camera border/speed correct.");
        }
         // Also check the legacy view borders for view 0
         if(view_get_hborder(0) != 0 || view_get_vborder(0) != 0) {
              show_debug_message("CONTROLLER STEP CHECK (WARNING): Legacy view_border[0] is NOT zero! H=" + string(view_get_hborder(0)) + ", V=" + string(view_get_vborder(0)) );
         }

    } else {
         show_debug_message("CONTROLLER STEP CHECK (ERROR): Global camera invalid/missing in Step!");
    }
}
...
if (instance_exists(global.player_instance)) {
    show_debug_message("Player: (" + string(global.player_instance.x) + ", " + string(global.player_instance.y) + ")");
    show_debug_message("Camera: (" + string(camera_get_view_x(global.camera)) + ", " + string(camera_get_view_y(global.camera)) + ")");
}